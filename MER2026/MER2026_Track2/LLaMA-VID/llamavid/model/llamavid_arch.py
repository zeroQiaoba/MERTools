...
class LLaMAVIDMetaModel:
    ...
    def init_bert(self, vision_width, cross_attention_freq=2, truncation_side="right"):
        ...
        tokenizer = BertTokenizer.from_pretrained(config.PATH_TO_MLLM['bert-base-uncased'], truncation_side=truncation_side)
        tokenizer.add_special_tokens({"bos_token": "[DEC]"})
        # initialize BERT
        encoder_config = BertConfig.from_pretrained(config.PATH_TO_MLLM['bert-base-uncased'])
        ...
        if "qformer" in self.config.bert_type:
            mm_model = BertLMHeadModelQF.from_pretrained(
                config.PATH_TO_MLLM['bert-base-uncased'], config=encoder_config
            )
