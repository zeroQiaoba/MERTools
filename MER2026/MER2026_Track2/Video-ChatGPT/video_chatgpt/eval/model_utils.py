...
def initialize_model(model_name, mm_vision_tower, projection_path=None):
    """
    ...
    model.config.mm_vision_tower = mm_vision_tower
    ...
    # Add tokens to tokenizer [增加了视觉相关的三个新的token]
    ...
    model = model.eval()
    model = model.cuda() # cpu 专属
    # model = model.float()  # cpu 专属 => 只有cuda才支持float16

    vision_tower_name = mm_vision_tower
