...
def func_name_conversion(audio, suffix):
    ...
    if os.path.exists(os.path.join(config.PATH_TO_FEATURES['MER2025'], audio1)):
        audio = audio1
    else:
        audio = audio2
    return audio
...
def feature_extraction_progress(dataset_name):
    feature_dir = config.PATH_TO_FEATURES[dataset_name]
    for subdir in glob.glob(feature_dir + '/*'):
        featname = os.path.basename(subdir)
        filenums = len(os.listdir(subdir))
        print (featname, filenums)
