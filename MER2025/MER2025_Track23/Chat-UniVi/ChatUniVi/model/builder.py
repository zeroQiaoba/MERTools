...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto"):
    ...
    if 'chatunivi' in model_name.lower():
        ...
        else: # load model using this manner !!
            tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)
            model = AutoModelForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs) # 因为有cuda，他自动取加载cuda version的模型
    else:
        # Load language model
    ...
    if 'chatunivi' in model_name.lower():
        ...
        if not vision_tower.is_loaded:
            ...
            vision_tower.to(device='cuda', dtype=torch.float16) # cpu 专享
