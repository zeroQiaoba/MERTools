...
if __name__ == '__main__':
    ...
    # 1. read ground truth
    label_csv = config.PATH_TO_LABEL['MER2025OV']
    name2gt = {}
    ...
    for result_path in glob.glob('output/results-mer2025ov/*/*.npz'):
        ...
        level1_fscore, _, _ = wheel_metric_calculation(name2gt=name2gt, name2pred=name2pred, inter_print=False, level='level1')
        level2_fscore, _, _ = wheel_metric_calculation(name2gt=name2gt, name2pred=name2pred, inter_print=False, level='level2')
        print_scores = [level1_fscore, level2_fscore, np.mean([level1_fscore, level2_fscore])]
        print_scores = '& '.join(['%.2f' %(item*100) for item in print_scores])
        print (modelname, print_scores)
