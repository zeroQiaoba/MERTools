...
def split_audio_from_video(video_path, audio_path):
    cmd = '%s -i \"%s\" -ac 1 -ar 16000 -loglevel quiet -y \"%s\"' %(PATH_TO_FFMPEG, video_path, audio_path)
    os.system(cmd)

def merge_audio_and_video(audio_path, video_path, save_path):
    cmd = '%s -i \"%s\" -i \"%s\" -vcodec copy -acodec copy -c:v copy -c:a aac -loglevel quiet -y \"%s\"' %(PATH_TO_FFMPEG, audio_path, video_path, save_path)
    os.system(cmd)
...
# python main-noise.py main_mixture_multiprocess --video_root='./dataset/video' --save_root='./tempvideo-noise' --debug=True
def main_mixture_multiprocess(video_root, save_root, debug=False):

    choice_blurs = [1, 2, 4]
    choice_snrs = np.arange(5, 11) # [5, 10]
    video_paths = glob.glob(video_root + '/*')
    noise_paths = glob.glob(PATH_TO_NOISE + '/*')
